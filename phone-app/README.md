...
Built, installed, and iterated on against a real Moto G phone over several
rounds of on-device testing — not just a compile-and-hope build. The call
screen in particular has been through multiple real-device debugging passes
(see "Non-obvious gotchas" below for what that surfaced). Mascot/gesture and
agent-state code is shared line-for-line with `tab-app` (same files, package
name swapped) — anything fixed there gets ported here in the same pass, not
...
(`minSdk = 26` in `app/build.gradle.kts`) — older phones can't install it:

> **[Download phone-app-debug.apk](https://github.com/sooriyapsn/baby-story-teller/releases/download/debug-latest/phone-app-debug.apk)**

Open that link in the phone's browser, let it download, then skip to
...
### Troubleshooting: "No connected devices!" / adb doesn't see the phone

This is the single most common snag with the USB + adb install path, and
it's almost always the same cause: **the phone is plugged in, but USB
debugging isn't turned on yet.** Plain USB file-transfer mode (what a
phone uses by default when you plug it in) and USB *debugging* mode (what
`adb` needs) are two different things — being able to see the phone's
files on your computer doesn't mean `adb` can talk to it.

Fix it in order — most people only need step 1–4:

1. **Unlock Developer Options** (skip if you've done this before): on the
   phone, go to **Settings → About phone**, find **Build number**, and
...
4. **Unlock the phone's screen and look at it.** A popup should appear:
   *"Allow USB debugging?"* with a long string of letters/numbers (your
   ...
   the lock screen — make sure the phone is unlocked and awake.
5. **Check it worked**, from a terminal on your computer:
   ```bash
   adb devices
   ```
   You want to see your phone listed with the word `device` next to it,
   like:
   ```
   ...
   - If it says `unauthorized` instead of `device` — you missed the popup
     in step 4; unlock the phone and look again.
   ...
   - Still nothing? Try a different USB cable or port. Some cheap cables
     ...
     the phone still charges through them.

Once `adb devices` shows your phone as `device`, run
...
`.github/workflows/android.yml` builds this app (and `tab-app`) on every
...
Give it its own tag (`phone-app-v2`, etc.) rather than reusing `debug-latest`,
