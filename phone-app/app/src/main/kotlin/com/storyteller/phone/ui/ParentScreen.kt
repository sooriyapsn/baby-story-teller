...
@Composable
fun ParentScreen(baseUrl: String, serverConfig: ServerConfig, onClose: () -> Unit) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(32.dp)) {
        ...
        if (pin == null) {
            ...
            SettingsForm(baseUrl = baseUrl, pin = pin!!, serverConfig = serverConfig)
        }
    }
}
...
@Composable
private fun SettingsForm(baseUrl: String, pin: String, serverConfig: ServerConfig) {
    ...
    var serverUrlText by remember { mutableStateOf(baseUrl) }
    var wakeSecretText by remember { mutableStateOf("") }
    var serverStatus by remember { mutableStateOf<String?>(null) }

    androidx.compose.runtime.LaunchedEffect(Unit) {
        wakeSecretText = serverConfig.wakeSecretFlow.first()
    }
    ...
    Column(modifier = Modifier.fillMaxWidth().padding(top = 16.dp)) {
        ...
        androidx.compose.material3.HorizontalDivider(modifier = Modifier.padding(vertical = 24.dp))

        Text("Server connection", style = MaterialTheme.typography.titleMedium)
        Text(
            "Changing this reconnects the app to a different server — the app restarts its connection immediately after saving.",
        ...
        OutlinedTextField(
            value = wakeSecretText,
            onValueChange = { wakeSecretText = it },
            label = { Text("Wake secret") },
            modifier = Modifier.fillMaxWidth().padding(top = 8.dp),
        )
        serverStatus?.let { Text(it, modifier = Modifier.padding(top = 8.dp)) }
        Button(
            onClick = {
                scope.launch {
                    serverConfig.setBaseUrl(serverUrlText)
                    serverConfig.setWakeSecret(wakeSecretText)
                    serverStatus = "Saved — reopen this app (or go back to the picker) to use the new address."
                }
            },
